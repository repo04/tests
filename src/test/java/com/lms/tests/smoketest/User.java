...
public class User extends BaseClass {
    ...
    String defaultTimeZoneExpectedValue = xpv.getTokenValue(programName + "TimeZoneDefaultValue");
    ...
    /**
     * Verify the default state of unmask password field field in Create User Page UI is unchecked
     */
    ...
    /**
     * Verify the different values in "Authentication Method" dropdown field in Create User Page
     */
    public void verifyChooseAnAuthenticationMethodDropDownFieldInCreateUserPage() {
        ...
        String[] dropdownValues={"CAS server (SSO)","External database","Email-based self-registration","FirstClass server","IMAP server","LDAP server","Manual accounts"
        ,"MNet authentication","NNTP server","No login","No authentication","PAM (Pluggable Authentication Modules)","POP3 server","RADIUS server"
        ,"Shibboleth","Web services authentication"};
        ...
        for(WebElement i: authenticationMethodDropdownValues.getOptions()) {  
            ...
            for (j=1; j<dropdownValues.length; j++){
                 if (value.equalsIgnoreCase(dropdownValues[j-1])) {
                     break;
                 }
            }
            if (!value.equalsIgnoreCase(dropdownValues[j-1])) {
               Utility.illegalStateException("Not Expected Value present in Choose An Authentication Method drop down: '" + value + "'");
            }
        }          
    }

...
public class SystemCompatibility extends BaseClass {
    ...
    String browserName = BaseClass.browser;
    ...
    public void mobileApplicationsUIVerify() {
        ...
        if (programName.equals("gu-msn") || programName.equals("unc-mba") || programName.equals("usc-mat") || programName.equals("usc-msw")) {
     ...
     */
    public void systemCompatiblityVerifyQuestionsAndBrowserCompatibleIcons() {
        ...
        //Store the SystemCompatibility (Current Window) in string "winHandleBefore"
        ...
        //The Window Title of new window opened on clicking FF image changes when opened in different browsers, so through the below code we are switching to the new window opened and depending on browser, we are cheking the download browser button present
        for (String winHandle : driver.getWindowHandles()) {
            driver.switchTo().window(winHandle);
        }

        // Perform the actions on new window depending upon the browser running the code
        if (browserName.equalsIgnoreCase("ff")) {
            new WebDriverWait(driver, 60).until(ExpectedConditions.elementToBeClickable(By.xpath(xpv.getTokenValue("downloadBtnFFXPATH"))));
        } else {
            new WebDriverWait(driver, 60).until(ExpectedConditions.elementToBeClickable(By.xpath(xpv.getTokenValue("downloadbtnXPATH"))));
        }

        //Switching back to original SystemCompatiblity window stored in string "winHandleBefore"
